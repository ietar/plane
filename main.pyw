import pygame,sys,traceback,myplane,enemy,bullet,supply,pickle
...
def Writeline(text='',font=r'font\fs.ttf',color=WHITE,bold=False,size=24,alter_x=None,alter_y=0):
...
def main():
    ...
    #标志声音播放/静音
    silence = False
    volume_on_image = pygame.image.load(r'images\volume_on.png').convert_alpha()
    volume_off_image = pygame.image.load(r'images\volume_off.png').convert_alpha()
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.button == 1 and volume_rect.collidepoint(event.pos): #鼠标在静音框内左键
                    silence = not silence
                    if silence:
        ...
        screen.blit(bomb_text,(10+bomb_rect.width,height-bomb_rect.height-13))
    ...
    def Shift_to_toplist():
        ...
        for i in range(len(new_record)):
            if new_record[i][0] == username and new_record[i][1]==score and not mark_me:
                ...
                Writeline(text=str(new_record[i][1]),alter_x=100,alter_y =-150+30*i)
